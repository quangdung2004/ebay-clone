...
const ProductDetailPage = () => {
    ...
    // Timer for time left
    useEffect(() => {
        if (!product || !product.isAuction || product.isEnded || product.status === 'ENDED' || product.status === 'SOLD') {
            return;
        }
            ...
            signalrService.onBidPlaced((data) => {
                if (data.productId === Number(id)) {
                    ...
                    if (user && data.bidderId !== user.id) {
                        showSuccess(`A new higher bid of ${formatCurrency(data.currentBid)} has been placed!`);
                    }
                }
            });
    ...
    const handlePlaceBid = async (e) => {
        ...
        const minBid = product.currentBid != null ? product.currentBid + 1 : product.price;
            ...
            <div className="pdp-container">
                <div className="pdp-skeleton">
                    ...
                    <div className="pdp-skeleton-info">
                        ...
                        <div className="skeleton-text" style={{ width: '30%', height: '1.2rem', marginTop: '1rem' }} />
                        <div className="skeleton-text" style={{ width: '100%', height: '4rem', marginTop: '1rem' }} />
                    </div>
                </div>
            </div>
        );
    ...
    const canBuy = !product.isAuction && product.inStock && !isOut && !isInactive && !isSold && !isOwner;
    const canBid = product.isAuction && !isEnded && !isInactive && !isSold && !isOut && !isOwner;
        ...
        <div className="pdp-container">
            <div className="pdp-layout">
                    ...
                    {images.length > 1 && (
                        <div className="pdp-gallery__thumbs">
                            {images.map((url, i) => (
                                <button
                                    key={i}
                                    className={`pdp-gallery__thumb ${i === selectedImg ? 'pdp-gallery__thumb--active' : ''}`}
                ...
                <div className="pdp-info">
                    ...
                    <div className="pdp-meta-stats">
                        <span className={`badge bg-${conditionLabel.color}`}>{conditionLabel.text}</span>
                        {product.isAuction ? (
                            <span className="badge bg-primary">Auction</span>
                        ) : (
                            <span className="badge bg-info">Buy It Now</span>
                        )}
                        <span className={`badge bg-${statusLabel.color}`}>{statusLabel.text}</span>
                    </div>
                            ...
                            <>
                                ...
                                <div className="pdp-stock-meta">
                                    {product.inStock && product.availableQuantity > 0 ? (
                                        <span className="text-success">{product.availableQuantity} available</span>
                                    ) : (
                    ...
                    <div className="pdp-seller-info">
                        ...
                        <div className="seller-name">{product.sellerName || `Seller #${product.sellerId}`}</div>
                        <div className="seller-stats">{product.viewCount} views &bull; {product.categoryName || `Category ${product.categoryId}`}</div>
                    </div>
                    ...
                    <div className="pdp-actions-panel">
                        {isOwner ? (
                            <div className="pdp-action-group">
                                <p className="text-muted" style={{ fontWeight: '500', marginBottom: 0 }}>This is your listing.</p>
                                <Link to={`/sell/edit/${product.id}`} className="pdp-btn-secondary" style={{ textAlign: 'center', display: 'block' }}>
                                    Edit Listing
                                </Link>
                            </div>
                        ) : product.isAuction ? (
                            <div className="pdp-bid-container">
                                {isEnded || isSold ? (
                                    <div className="pdp-ended-message">
                                        {product.status === 'SOLD' ? 'This auction has ended and the item was sold.' : 'This auction has ended without a winner.'}
                                ...
                                <div className="min-bid-hint">
                                    {!isEnded && !isSold && canBid && `Enter ${formatCurrency(product.currentBid != null ? product.currentBid + 1 : product.price)} or more`}
                                </div>
                            </div>
                        ) : (
                            <div className="pdp-action-group">
                                <button
                                    ...
                                    disabled={!canBuy}
                                >
                                    {canBuy ? 'Buy It Now' : 'Currently Unavailable'}
                                </button>
                                <button
            ...
            {product.isAuction && (
                <div className="pdp-bottom-section">
                    <BidHistory productId={product.id} currentBid={product.currentBid} lastBidTrigger={lastBidTrigger} />
                </div>
            )}
        </div>
    );
